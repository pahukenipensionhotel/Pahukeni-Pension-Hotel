...
import { X, Plus, CreditCard, History, AlertCircle } from "lucide-react";
...
export const ConferenceFolioModal = ({
  ...
  const handleCheckout = async () => {
    if (!confirm(`Confirm checkout for ${booking.room_name}? Total Balance Due: N$ ${folio?.balance_due}`)) return;
    setLoading(true);

    try {
      const batch = writeBatch(db);

      batch.update(doc(db, "conference_bookings", booking.id), { status: "Checked Out" });

      batch.update(doc(db, "conference_rooms", booking.room_id), { status: "Available" });
      ...
      await logger.info(
        ...
        `Client ${booking.client_name} checked out from ${booking.room_name}. Final balance settled.`,
        undefined,
        "Staff User",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-100">
      ...
      >
        <div className="p-8 border-b border-black/5 flex justify-between items-center bg-[#141414] text-white">
          <div>
            <h3 className="text-2xl font-serif italic">Conference Folio</h3>
            <p className="text-[10px] font-mono opacity-40 uppercase tracking-widest mt-1">
              {booking.room_name} • {booking.client_name}
            </p>
          </div>
