  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-closed-by-user') {
        ...
        setError('This domain is not authorized for Google login. Please add your Vercel domain to the "Authorized Domains" list in the Firebase Console (Authentication > Settings).');
      } else {
...
const POSModule = ({ type, menu, orders, isAdmin, userRole, user, createNotification }: { type: 'Restaurant' | 'Bar', menu: MenuItem[], orders: Order[], isAdmin: boolean, userRole?: string, user: User, createNotification: (notif: any) => Promise<void> }) => {
  ...
  const activeOrders = filteredOrders.filter(o => o.status !== 'Pending' && o.status !== 'Completed' && o.status !== 'Cancelled' && o.status !== 'Paid');
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-sm font-mono uppercase tracking-widest text-black/60">Received Orders</h3>
                    </div>
                    {activeOrders.length === 0 ? (
                      <div className="bg-white/50 p-8 rounded-2xl border border-dashed border-black/10 text-center">
                        <p className="text-xs text-black/30 font-mono">No orders in process</p>
                      </div>
                    ) : (
                          ...
                          <div className="space-y-1 mb-4 border-y border-black/5 py-3">
                            {order.items.map((item, idx) => (
                              <div key={idx} className="flex justify-between text-xs">
                                ...
                                <span className="font-mono opacity-40">N$ {item.price * item.qty}</span>
                              </div>
                            ))}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-sm font-mono uppercase tracking-widest text-black/60">Incoming Orders</h3>
                    </div>
                    {incomingOrders.length === 0 ? (
                      <div className="bg-white/50 p-8 rounded-2xl border border-dashed border-black/10 text-center">
                        <p className="text-xs text-black/30 font-mono">No new orders</p>
                      </div>
                    ) : (
                      incomingOrders.map((order) => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              ...
                              <p className="text-[10px] font-mono text-orange-600/60 uppercase">
                                Just arrived • {new Date(order.created_at).toLocaleTimeString()}
                              </p>
                            </div>
                          ...
                          <div className="space-y-1 mb-4 border-y border-orange-200/30 py-3">
                            {order.items.map((item, idx) => (
                              <div key={idx} className="flex justify-between text-xs text-orange-800/80">
                                <span>{item.name} x {item.qty}</span>
                                <span className="font-mono opacity-60">N$ {item.price * item.qty}</span>
                              </div>
                            ))}
                          </div>
  ...
  useEffect(() => {
    ...
    allOrders.forEach(order => {
      ...
      if (prevStatus && prevStatus !== order.status) {
        const type = 'type' in order ? `${order.type} Order` : 'Laundry Service';
